...
const uploadFile = asyncHandler(async (req, res) => {
    const { file, file_type } = req.files

    try {
        const newFileName = `${uuidv4()}__${(new Date).toISOString()}` + path.extname(image_file.name);
        
        const is_uploaded = fileUploader(file, newFileName)
        if (!is_uploaded) {
            return res.status(500).json({ 
                status: 'Failure',
...
const uploadMultipleFiles = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // Binary data base64
        // const uploadMul = multer({storage: MULTER_S3_CONFIG}).array('file', 4)
